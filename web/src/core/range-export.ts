 ...
 * 그 구간이 파일 여섯 개에 걸쳐 있으면 여섯 개를 이어 하나로 만든다.
 *
 * 영상은 다시 인코딩하지 않는다. Annex-B 스트림을 잘라 붙일 뿐이다.
 * 그래서 **반드시 키프레임에서 시작**해야 한다 — 중간 프레임부터 붙이면
 * 참조할 앞 프레임이 없어 깨진다. 시작 지점 직전 키프레임까지 거슬러 올라간다.
 *
...
export type RangeKind = 'front' | 'rear' | 'audio' | 'gps' | 'sensor';

export const RANGE_LABEL: Record<RangeKind, string> = {
  front: '전방 영상', rear: '후방 영상', audio: '음성', gps: 'GPS', sensor: 'G센서',
};

/** 파일명에 붙는 이름. 사람이 보고 무엇인지 알 수 있어야 한다. */
const RANGE_SUFFIX: Record<RangeKind, string> = {
  front: 'Front.h264', rear: 'Rear.h264', audio: 'Audio.wav', gps: 'GPS.csv', sensor: 'Sensor.csv',
};
...
export function estimateRangeBytes(segments: SegmentInfo[], range: TimeRange, kind: RangeKind): number {
  ...
  for (const s of segmentsInRange(segments, range)) {
    ...
    total += s.size * 0.95 * (mine / frames) * share;
  }
...
/**
 * 이 구간에서 내보낼 영상 패킷 번호들.
 *
 * 패킷 시각은 절대 벽시계다. 다시 인코딩하지 않으므로 **시작 지점 직전
 * 키프레임까지 거슬러 올라가야** 한다 — 중간 프레임부터 붙이면 참조할 앞
 * 프레임이 없어 화면이 깨진다. 그만큼 요청한 시각보다 몇 초 이르게 시작한다.
 */
function pickVideo(doc: JdrDocument, channel: number, range: TimeRange): number[] {
  const p = doc.packets;
  ...
  for (let i = Math.max(0, startKey); i < p.count; i++) {
    ...
    // 앞에 키프레임이 아예 없으면 (파일이 잘렸거나 하면) 범위 안만 담는다
...
export async function buildRange(
  ...
  for (let n = 0; n < list.length; n++) {
    ...
    const picked = kind === 'audio'
      ? pickByKind(doc, TagKind.Audio, range)
      : pickVideo(doc, kind === 'front' ? 0 : 1, range);
  ...
  return done(out.finish(undefined, 'video/h264'));
}
