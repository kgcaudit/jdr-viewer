...
async function cachePut(key: string, addr: string): Promise<void> {
  const db = await openDb();
  if (!db) return;
  try {
    const tx = db.transaction(STORE, 'readwrite');
    tx.objectStore(STORE).put(addr, key);
  } catch { /* 다음에 다시 부른다 */ }
}
...
/**
 * 좌표의 주소를 돌려준다. 캐시에 있으면 즉시, 없으면 큐에 넣어 부른다.
 * 실패하면 빈 문자열. 같은 좌표를 연달아 물으면 한 번만 부른다.
 */
